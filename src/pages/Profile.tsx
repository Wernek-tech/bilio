...
export default function Profile() {
  ...
  return <SiteShell><div className="page-body profile-page-final">
    <section className="profile-header">
      ...
      <div className="profile-social-region"><div className="profile-like own-like" title="Profil beğenileri"><span>♡</span><b>{format(profile.likeCount)}</b><small>BEĞENİ</small></div><div className="friends-region"><header><h2>ARKADAŞLARIM</h2><span>{friends.filter(friend => friend.online).length} çevrim içi</span></header><div>{friends.length ? friends.slice(0, 6).map(friend => <article key={friend.userId}><div className="friend-avatar">{friend.avatarUrl ? <img src={friend.avatarUrl} alt=""/> : friend.username.slice(0, 1).toLocaleUpperCase('tr-TR')}</div><p><b>{friend.username}</b><small className={friend.online ? 'online' : ''}>{friend.online ? 'Çevrim içi' : 'Çevrim dışı'} · {friend.mutualCount} ortak arkadaş</small></p></article>) : <p className="friends-empty">Henüz arkadaşın bulunmuyor. Lobideki oyuncuları arkadaş olarak ekleyebilirsin.</p>}</div></div></div>
    </section>
