...
type ProfileData = {
  ...
  userId: string; likeCount: number; likedByMe: boolean; isSelf: boolean;
...
type Friend = {userId: string; username: string; avatarUrl: string; frameId?:string|null; titleId?: string; online: boolean; mutualCount: number};
...
export default function Profile() {
  ...
  useEffect(() => { if (!userId) return; void load(); const timer=window.setInterval(()=>{void api<{items:Friend[]}>('/friends').then(result=>setFriends(result.items)).catch(()=>undefined)},30_000);return()=>window.clearInterval(timer); }, [load, userId]);
  useEffect(()=>{if(!messageFriend)return;const friendId=messageFriend.userId;const timer=window.setInterval(()=>{void api<{items:PrivateMessage[]}>(`/private-messages/${encodeURIComponent(friendId)}`).then(result=>setPrivateMessages(result.items)).catch(()=>undefined)},3_000);return()=>window.clearInterval(timer)},[messageFriend]);

  const openMessages = async (friend: Friend) => {
    setError(''); setActiveFriend(null); setMessageFriend(friend); setPrivateText('');
    try { const result=await api<{items:PrivateMessage[]}>(`/private-messages/${encodeURIComponent(friend.userId)}`);setPrivateMessages(result.items); }
    catch(reason){setMessageFriend(null);setError(reason instanceof Error?reason.message:'Mesajlar açılamadı.');}
  };
  const removeFriend = async (friend: Friend) => {await api('/friends/remove',{method:'POST',body:JSON.stringify({userId:friend.userId})});setActiveFriend(null);await load();};
  const blockFriend = async (friend: Friend) => {await api(`/users/${encodeURIComponent(friend.userId)}/block`,{method:'POST'});setActiveFriend(null);await load();};
  const sendPrivateMessage = async () => {if(!messageFriend||!privateText.trim()||busy)return;setBusy(true);try{const result=await api<{item:PrivateMessage}>(`/private-messages/${encodeURIComponent(messageFriend.userId)}`,{method:'POST',body:JSON.stringify({content:privateText})});setPrivateMessages(items=>[...items,result.item]);setPrivateText('');}catch(reason){setError(reason instanceof Error?reason.message:'Mesaj gönderilemedi.');}finally{setBusy(false)}};
  ...
  return <SiteShell><div className="page-body profile-page-final">
    <section className="profile-header">
      ...
      <div className="profile-social-region"><div className="profile-like own-like" title={`${format(profile.likeCount)} profil beğenisi`}><span aria-hidden="true">♡</span><b>{format(profile.likeCount)}</b><small>BEĞENİ</small></div><div className="friends-region"><header><h2>ARKADAŞLARIM</h2><span>{friends.filter(friend => friend.online).length} çevrim içi</span></header><div>{friends.length ? friends.slice(0, 6).map(friend => <article className="friend-entry" key={friend.userId}><button className="friend-entry-main" onClick={()=>setActiveFriend(activeFriend?.userId===friend.userId?null:friend)}><PlayerAvatar className="friend-avatar" username={friend.username} avatarUrl={friend.avatarUrl} frameId={friend.frameId}/><p><b>{friend.username}</b><small className={friend.online ? 'online' : ''}>{friend.online ? 'Çevrim içi' : 'Çevrim dışı'} · {friend.mutualCount} ortak arkadaş</small></p><i className={friend.online?'online':''}/></button>{activeFriend?.userId===friend.userId&&<div className="friend-actions"><button onClick={()=>location.assign(`/profil/${friend.userId}`)}>PROFİLİ GÖRÜNTÜLE</button><button onClick={()=>void openMessages(friend)}>MESAJ GÖNDER</button><button onClick={()=>void removeFriend(friend)}>ARKADAŞTAN ÇIKAR</button><button className="danger" onClick={()=>void blockFriend(friend)}>ENGELLE</button></div>}</article>) : <p className="friends-empty">Henüz arkadaşın bulunmuyor. Lobideki oyuncuları arkadaş olarak ekleyebilirsin.</p>}</div></div></div>
    </section>
  ...
  {edit && <div className="modal-back" onMouseDown={event => {if (event.target === event.currentTarget && !busy) setEdit(false);}}><div className="profile-edit-modal compact modern-profile-editor" role="dialog" aria-modal="true" aria-labelledby="edit-profile-title"><header><div><small>PROFİLİNİ KİŞİSELLEŞTİR</small><h2 id="edit-profile-title">Profil fotoğrafı ve hakkında</h2></div><button aria-label="Pencereyi kapat" onClick={()=>setEdit(false)}>×</button></header>{cropSource ? <div className="avatar-crop-editor"><div className="crop-hint">Fotoğrafı sürükleyerek konumlandır</div><div className="avatar-crop-window" onPointerDown={event=>{event.currentTarget.setPointerCapture(event.pointerId);drag.current={x:crop.x,y:crop.y,startX:event.clientX,startY:event.clientY}}} onPointerMove={event=>{if(!drag.current)return;setCrop(current=>({...current,x:Math.max(-120,Math.min(120,drag.current!.x+event.clientX-drag.current!.startX)),y:Math.max(-120,Math.min(120,drag.current!.y+event.clientY-drag.current!.startY))}))}} onPointerUp={()=>{drag.current=null}}><span className="crop-grid"/><img src={cropSource} alt="Profil resmi kırpma ön izlemesi" style={{transform: `translate(${crop.x}px, ${crop.y}px) scale(${crop.scale})`}}/></div><label><span>YAKINLAŞTIR</span><input type="range" min="1" max="3" step="0.01" value={crop.scale} onChange={event => setCrop(current => ({...current, scale: Number(event.target.value)}))}/><b>{Math.round(crop.scale*100)}%</b></label><div className="crop-position"><button type="button" onClick={() => setCrop({scale: 1, x: 0, y: 0})}>↺ ORTALA</button><button type="button" onClick={() => document.getElementById('avatar-file')?.click()}>↥ BAŞKA FOTOĞRAF</button></div></div> : <div className="profile-edit-preview"><ProfileAvatar profile={profile} previewUrl={avatar} frameId={profile.selectedFrameId}/><p>JPG, PNG veya WEBP · En fazla 5 MB</p></div>}<label>Günün sözü / Hakkımda<textarea maxLength={240} value={draft.about} onChange={event => setDraft({about: event.target.value})} placeholder="Bugün ne hissediyorsun?"/><small>{draft.about.length}/240</small></label><label className="avatar-upload-label">Profil resmi<input id="avatar-file" className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={event => void onFile(event)}/>{!cropSource&&<button type="button" onClick={() => document.getElementById('avatar-file')?.click()}>＋ PROFİL FOTOĞRAFI SEÇ</button>}<span>{fileName || 'Yeni bir resim seçilmedi.'}</span></label>{error && <div className="inline-error">{error}</div>}<div className="modal-actions"><button onClick={() => setEdit(false)} disabled={busy}>İPTAL</button><button className="save-profile" onClick={() => void saveProfile()} disabled={busy}>{busy ? 'KAYDEDİLİYOR…' : 'DEĞİŞİKLİKLERİ KAYDET'}</button></div></div></div>}
  {messageFriend&&<div className="modal-back" onMouseDown={event=>{if(event.target===event.currentTarget)setMessageFriend(null)}}><section className="private-message-modal" role="dialog" aria-modal="true" aria-labelledby="private-message-title"><header><div className="friend-avatar">{messageFriend.avatarUrl?<img src={messageFriend.avatarUrl} alt=""/>:messageFriend.username.slice(0,1).toLocaleUpperCase('tr-TR')}</div><div><h2 id="private-message-title">{messageFriend.username}</h2><small className={messageFriend.online?'online':''}>{messageFriend.online?'Çevrim içi':'Çevrim dışı'}</small></div><button aria-label="Mesaj penceresini kapat" onClick={()=>setMessageFriend(null)}>×</button></header><div className="private-message-history">{privateMessages.length?privateMessages.map(item=><p className={item.senderId===profile.userId?'mine':''} key={item.id}><span>{item.content}</span><time>{new Date(item.createdAt).toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'})}</time></p>):<div className="private-message-empty">Henüz mesaj yok. İlk mesajı sen gönderebilirsin.</div>}</div><form onSubmit={event=>{event.preventDefault();void sendPrivateMessage()}}><textarea maxLength={1000} value={privateText} onChange={event=>setPrivateText(event.target.value)} placeholder="Özel mesajını yaz…"/><button disabled={busy||!privateText.trim()}>GÖNDER</button></form>{error&&<div className="inline-error">{error}</div>}</section></div>}
  </SiteShell>;
}
