...
export default function Lobby() {
    ...
    const load = useCallback(async () => { try {
        ...
        const recent=messages.items.filter(item => item && item.id).slice(-40);
        const profileEntries=await Promise.all([...new Set(recent.filter(item=>item.userId&&!item.userId.startsWith('bot-')).map(item=>item.userId!))].map(async id=>{const result=await api<{profile:{selectedFrameId:string|null}}>(`/profiles/${encodeURIComponent(id)}`).catch(()=>null);return [id,result?.profile.selectedFrameId||null] as const;}));
        const frameByUser=new Map(profileEntries);
        setItems(recent.map(item=>({...item,frameId:item.frameId||frameByUser.get(item.userId||'')||null})));
        if (userId) {
