// ponytail: ported near-verbatim from live bilio (v15) at the user's explicit request — this page
// stays in the OLD visual style (legacy.css), not restyled to match Bolt. Only change from the
// original: SiteShell wrapper removed (Bolt's App already provides the shell) and auth/api swapped
// for bolt-migration's own helpers.
...
export function Leaderboard({ myUserId }: { myUserId: string }) {
  ...
  return <div className="page-body leaderboard-page">
    {loading ? <div className="page-loading">Sıralama yükleniyor...</div> : error ? <div className="leaderboard-empty"><span className="small-trophy">♜</span><b>Sıralama yüklenemedi.</b><small>{error}</small></div> : rows.length === 0 ? <section className="rank-table leaderboard-empty-table" aria-label="Haftalık liderlik tablosu">
      ...
      {rest.length > 0 && <section className="rank-table leaderboard-rest-v10" aria-label="Haftalık liderlik tablosu">
        <header><span>SIRA</span><span>OYUNCU</span><span>PUAN</span></header>
        ...
        {mine && !rows.some((row) => row.userId === mine.userId) && <div className="rank-row pinned"><span>{mine.rank}</span><PlayerIdentity row={mine} /><strong>{format(mine.score)}</strong></div>}
        {!mine && <div className="no-rank">Bu hafta henüz sıralama puanın bulunmuyor.</div>}
      </section>}
    </>}
  </div>;
}
